...
namespace Turbo.Plugins.Razor.RunStats
{
	...
	public class MenuGreaterRift : BasePlugin, IMenuAddon, IAfterCollectHandler/*, INewAreaHandler, IInGameTopPainter, ILeftClickHandler, IRightClickHandler*/
	{
		public bool HideWhileNephalemRiftOpen { get; set; } = true;
		...
		private DateTime? StartTime; //when the plugin started recording its first rift
		...
		private long TotalCompletionTicks = 0;
		...
		public IFont TimeFont2 { get; set; }
		...
		private uint[] NephalemRiftQuestIds = new uint[] {1, 3, 10, 5};
		...
		private bool NephalemRiftOpen = false;
		...
		public class RiftInfo {
			...
			public uint GuardianKillStep { get; set; }
		...
		public void OnRegister(MenuPlugin plugin)
		{
			...
			TimeFont2 = Hud.Render.CreateFont("tahoma", plugin.FontSize, 255, 150, 150, 150, false, false, true);
			...
			DifficultyFont = Hud.Render.CreateFont("tahoma", plugin.FontSize, 255, 198, 86, 255, false, false, true);
			...
			ProgressBrush = Hud.Render.CreateBrush(100, 198, 86, 255, 0); //235, 211, 52
			
			Timer = Hud.Time.CreateWatch();
			UpdateTimer = Hud.Time.CreateWatch();
			...
			Label = new LabelRowDecorator(Hud,
				...
				new LabelTextureDecorator(Hud, Hud.Texture.GetTexture("Marker_Portal_Purple")) {TextureHeight = 34, ContentHeight = plugin.MenuHeight, ContentWidth = 18}//,
			); //{SpacingLeft = 15, SpacingRight = 5}; //icon spacing offset
			LabelHint = plugin.CreateHint("Greater Rift History");
			...
			TableUI = new LabelTableDecorator(Hud, 
				new LabelRowDecorator(Hud,
					...
					//game difficulty
					new LabelStringDecorator(Hud) {Hint = plugin.CreateHint("Tier"), Font = DifficultyFont, SpacingLeft = 6, SpacingRight = 6},
					...
					//boss icon
					new LabelTextureDecorator(Hud) {Hint = plugin.CreateHint("Boss"), TextureWidth = 30, TextureHeight = 30, ContentHeight = plugin.MenuHeight, SpacingLeft = 2, SpacingRight = -4},
					//boss name
					...
					//close time
					new LabelStringDecorator(Hud) {Hint = plugin.CreateHint("Time To Close"), Font = TimeFont, Alignment = HorizontalAlign.Left, SpacingLeft = 6, SpacingRight = 6},
					//xp gained
				...
				OnFillRow = (row, index) => {
					...
					foreach (RiftShrineInfo shrine in rift.Shrines.Values)
					{
						if (i < shrinesUI.Count)
						{
							...
							if (!shrine.Type.HasValue) //placeholder
							{
								...
								icon.ContentWidth = 22; //icon.ContentWidth = plugin.MenuHeight*0.8f + icon.SpacingLeft + icon.SpacingRight; //22;
							}
							else
							{
								icon.Texture = GetBuffTexture(shrine.Type.Value);
								icon.TextureWidth = plugin.MenuHeight - 2;//plugin.MenuHeight*0.8f;
								icon.TextureHeight = plugin.MenuHeight - 2; //plugin.MenuHeight*0.8f; //progressUI.Height + 2; //plugin.MenuHeight;
								icon.SpacingTop = 0;
								icon.ContentHeight = plugin.MenuHeight; //progressUI.Height + 2;
								icon.ContentWidth = plugin.MenuHeight; //icon.ContentWidth + icon.SpacingLeft + icon.SpacingRight; //plugin.MenuHeight; //22;
							}
					...
					//boss icon
					((LabelTextureDecorator)row.Labels[4]).Texture = rift.GuardianKilledAt > 0 ? Hud.Texture.GetTexture(3692681898) : Hud.Texture.GetTexture("WaypointMap_MarkerBoss"); //, Hud.Texture.GetTexture("WaypointMap_MarkerBoss")
					...
					//close time
					completionUI = (LabelStringDecorator)row.Labels[8];
					if (rift.CloseTicks < 0)
					{
						if (rift.EndedAt == -1)
						{
							completionUI.Font = RedFont;
						...
						else
						{
							//in progress
							completionUI.Font = TimeFont;
							completionUI.StaticText = ValueToString((long)(Hud.Game.CurrentGameTick + rift.CloseTicks) * 1000 * TimeSpan.TicksPerMillisecond / 60, ValueFormat.LongTime);
						}
					}
					else if (rift.CloseTicks > 0)
					{
						completionUI.Font = GreenFont;
						completionUI.StaticText = ValueToString((long)rift.CloseTicks * 1000 * TimeSpan.TicksPerMillisecond / 60, ValueFormat.LongTime);
					}
					else if (rift.CloseTicks == 0)
					{
						if (rift.EndedAt == -1)
					...
					//xp gained
					((LabelStringDecorator)row.Labels[9]).StaticText = ValueToString(rift.ExperienceGained, ValueFormat.ShortNumber) + " xp";
			...
			SummaryUI = new LabelAlignedDecorator(Hud,
				...
				new LabelRowDecorator(Hud,
					new LabelStringDecorator(Hud, () => AvgCompletionTicks == 0 ? TextIncomplete : ValueToString((long)AvgCompletionTicks * 1000 * TimeSpan.TicksPerMillisecond / 60, ValueFormat.LongTime)) {Hint = plugin.CreateHint(() => "Average Completion Time ("+Completed.Count+" Rifts)"), Font = GreenFont, SpacingRight = 1}, //"Average Completion Time ("+Completed.Count+" Rifts)" //TextIncomplete
					new LabelStringDecorator(Hud, "🕓") {Hint = plugin.CreateHint("Average Completion Time"), Font = TimeFont},
					new LabelStringDecorator(Hud, () => CountCompleted <= HistoryShown ? string.Empty : ValueToString((long)(TotalCompletionTicks / (float)CountCompleted) * 1000 * TimeSpan.TicksPerMillisecond / 60, ValueFormat.LongTime)) {Hint = plugin.CreateHint(() => "Average Completion Time ("+CountCompleted+" Rifts)"), Font = TimeFont2, SpacingLeft = 1} //, SpacingLeft = 2
				) {Alignment = HorizontalAlign.Right, SpacingLeft = 5} //Hint = plugin.CreateHint(() => Completed.Count == 0 ? "Average Completion Time" : "Average Completion Time ("+Completed.Count+" Rifts)"), 
			) {
				BackgroundBrush = plugin.BgBrush, 
				SpacingLeft = 10, 
				SpacingRight = 10,
			...
			Panel = new LabelColumnDecorator(Hud, 
				new LabelDelayedDecorator(Hud,
					new LabelAlignedDecorator(Hud, 
						new LabelStringDecorator(Hud, "GREATER RIFT HISTORY") {Font = plugin.TitleFont, SpacingLeft = 10, SpacingRight = 10},
		...
		{
			//listen for potential late update of xp gained from rift close
			if (UpdateTimer is object && UpdateTimer.IsRunning)
			{
				if (UpdateTimer.ElapsedMilliseconds < 1000)
				{
					if (RiftClosed is object)
						RiftClosed.ExperienceGained += GetExperienceGained();
				}
				else
				{
					UpdateTimer.Stop();
			...
			IQuest riftQuest = Hud.Game.Quests.FirstOrDefault(q => q.SnoQuest.Sno == 337492); // && q.State == QuestState.started //|| q.State == QuestState.completed) && QuestStepIds.Contains(q.QuestStepId)) ;
			if (riftQuest is object)
			{
				if (NephalemRiftQuestIds.Contains(riftQuest.QuestStepId))
				{
					NephalemRiftOpen = true;
				...
				if (RiftOpen == null)
				{
					...
					//if (RiftClosed == null || riftQuest.CreatedOn != RiftClosed.StartedAt)
					{
						...
						RiftOpen = new RiftInfo()
						{
							...
							//Difficulty = Hud.Game.Players.Max(p => p.InGreaterRiftRank), //Hud.Game.GameDifficulty, // Hud.Game.Me.InGreaterRiftRank
				...
				else
				{
					...
					/*if ((RiftOpen.LastSeenStep != riftQuest.QuestStepId) ||
						(riftQuest.QuestStepId == 1 && RiftOpen.StartedAt != riftQuest.CreatedOn) ||
						(riftQuest.QuestStepId == 3 && RiftOpen.GuardianSpawnedAt != riftQuest.CreatedOn) ||
						(riftQuest.QuestStepId == 10 && RiftOpen.GuardianKilledAt != riftQuest.CreatedOn) ||
						(riftQuest.QuestStepId == 5 && RiftOpen.CompletedAt != riftQuest.CreatedOn))
					{
						CloseRiftInfo();
						return;
					}*/
					...
					if (RiftOpen.Difficulty == 0)
					{
						var player = Hud.Game.Players.FirstOrDefault(p => p.InGreaterRiftRank > 0);
						if (player is object)
							RiftOpen.Difficulty = player.InGreaterRiftRank;
					}
							...
							if (marker.IsPylon || marker.IsShrine) //&& !RiftOpen.Shrines.ContainsKey(marker.Id) && ActorSnoEnumToShrineType.ContainsKey(marker.SnoActor.Sno))
							{
								if (!RiftOpen.Shrines.ContainsKey(marker.Id))
								{
									...
									if (marker.SnoActor.Sno == ActorSnoEnum._shrine_global) //placeholder
										RiftOpen.Shrines.Add(marker.Id, new RiftShrineInfo(marker, null/*ShrineType.HealingWell*/, marker.SnoActor.NameLocalized, Hud.Game.CurrentGameTick, Hud.Game.RiftPercentage, Hud.Game.Me.SnoArea?.NameLocalized));
									else
								...
								else if (!RiftOpen.Shrines[marker.Id].Type.HasValue /*ShrineType.HealingWell*/ && ActorSnoEnumToShrineType.ContainsKey(marker.SnoActor.Sno))
								{
									//Hud.Sound.Speak(Hud.Game.Me.SnoArea?.NameLocalized);
								...
								/*if (marker.SnoActor.Sno == ActorSnoEnum._shrine_global)
								{
									if (!RiftOpen.Shrines.ContainsKey(marker.Id))
										RiftOpen.Shrines.Add(marker.Id, new RiftShrineInfo(marker, ActorSnoEnumToShrineType[marker.SnoActor.Sno], marker.SnoActor.NameLocalized, Hud.Game.CurrentGameTick, Hud.Game.RiftPercentage, Hud.Game.Me.SnoArea?.NameLocalized));
								}
								else if (ActorSnoEnumToShrineType.ContainsKey(marker.SnoActor.Sno))
								{
									Hud.Sound.Speak("Add");
									RiftOpen.Shrines.Add(marker.Id, new RiftShrineInfo(marker, ActorSnoEnumToShrineType[marker.SnoActor.Sno], marker.SnoActor.NameLocalized, Hud.Game.CurrentGameTick, Hud.Game.RiftPercentage, Hud.Game.Me.SnoArea?.NameLocalized));
								*/
					...
					switch (riftQuest.QuestStepId)
					{
						...
						case 16: // Guardian spawned
							...
							if (Hud.Game.SpecialArea == SpecialArea.GreaterRift && string.IsNullOrEmpty(RiftOpen.GuardianName))
							{
						...
						case 34: // Guardian killed before timer ran out
							...
							if (RiftOpen.CloseTicks == 0)
							{
								RiftOpen.CloseTicks = Hud.Game.CurrentGameTick * -1; //Hud.Game.CurrentGameTick - (int)(((float)riftQuest.CompletedOn.ElapsedMilliseconds / 1000f) * 60);
								RiftOpen.LastSeenStep = riftQuest.QuestStepId;
							}
							
							if (RiftOpen.GuardianKillStep == 0)
								RiftOpen.GuardianKillStep = 34;
							...
							break;
						case 46: // Guardian killed after timer ran out or gem upgrades completed
							if (RiftOpen.GuardianKilledAt == 0)
							{
								RiftOpen.GuardianKilledAt = riftQuest.CreatedOn; //Hud.Game.CurrentGameTick;
								RiftOpen.LastSeenStep = riftQuest.QuestStepId;
		...
		private void CloseRiftInfo()
		{
			...
			if (RiftOpen.CloseTicks < 0)
				RiftOpen.CloseTicks = Hud.Game.CurrentGameTick + RiftOpen.CloseTicks;
			...
			else
			{
				...
				++CountCompleted;
				TotalCompletionTicks += RiftOpen.CompletedAt - RiftOpen.StartedAt;
