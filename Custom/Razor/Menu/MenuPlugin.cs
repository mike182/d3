namespace Turbo.Plugins.Razor.Menu
{
	...
	public class MenuPlugin : BasePlugin, IBeforeRenderHandler, IInGameTopPainter, ILeftClickHandler, ICustomizer, IMovable, ITextLogger, ILeftBlockHandler //, IBeforeRenderHandler
	{
        ...
        public override void Load(IController hud)
        {
			...
			TooltipFont = Hud.Render.CreateFont("tahoma", 7f, 255, 255, 255, 255, false, false, 180, 0, 0, 0, true);
			...
			LeftClickBlockCondition = () => Mover is object && Mover.EditMode && Docks.Values.Any(d => d.Visible && d.HoveredAddon is object && d.HoveredAddon.Label.Hovered);
		...
		public void OnRegister(MovableController mover)
		{
			...
			MenuHeight = (float)System.Math.Ceiling(Hud.Window.Size.Height - (potionUI.Rectangle.Bottom + 1)) + 1f;
			...
			if (Docks == null)
			{
				...
				//var inventoryUI = Hud.Inventory.InventoryMainUiElement;
				//var backpackUI = Hud.Render.GetUiElement("Root.NormalLayer.inventory_dialog_mainPage.inventory_button_backpack");
				//var objectivesUI = Hud.Render.GetUiElement("Root.NormalLayer.eventtext_bkgrnd.eventtext_region");
				var minimapUI = Hud.Render.MinimapUiElement;
				
				Docks = new Dictionary<string, IMenuDock>()
				{
					{ "BottomRight", new HorizontalMenuDock(Hud) { 
							LabelBrush = BgBrush,
							LabelHoveredBrush = BgBrushAlt,
							LabelPinnedBrush = PinnedBrush,
							Anchor = () => new RectangleF(potionUI.Rectangle.Right, potionUI.Rectangle.Bottom, Hud.Window.Size.Width - potionUI.Rectangle.Right, MenuHeight), //new RectangleF(Hud.Window.Size.Width*0.5f, Hud.Window.Size.Height*0.5f, 300, MenuHeight), //
							Alignment = HorizontalAlign.Right,
					...
					{ "BottomLeft", new HorizontalMenuDock(Hud) { 
							...
							LabelPinnedBrush = PinnedBrush,
							Anchor = () => {
								float x = Hud.Window.Size.Width * (Hud.Game.Me.HeroIsHardcore ? 0.13f : 0.087f); //0.087f : 0.043f
								return new RectangleF(x, potionUI.Rectangle.Bottom + 1, healthBall.Rectangle.Right - x, MenuHeight);
							}, 
					...
					{ "BottomCenter", new HorizontalMenuDock(Hud) { 
							...
							LabelHoveredBrush = BgBrushAlt,
							LabelPinnedBrush = PinnedBrush,
							Anchor = () => new RectangleF(healthBall.Rectangle.Right, bottomUI.Rectangle.Top + (bottomUI.Rectangle.Height * 0.335f), bottomCenterWidth, MenuHeight), //manaBall.Rectangle.Left - healthBall.Rectangle.Right, MenuHeight), 
					...
					{ "MinimapTop", new HorizontalMenuDock(Hud) { 
							...
							LabelPinnedBrush = PinnedBrush,
							LabelHoveredBrush = BgBrushAlt,
							Anchor = () => new RectangleF(minimapUI.Rectangle.X, minimapUI.Rectangle.Y, minimapUI.Rectangle.Width*0.85F, MenuHeight), 
							Alignment = HorizontalAlign.Left,
					...
					{ "MinimapBottom", new HorizontalMenuDock(Hud) { 
							...
							LabelHoveredBrush = BgBrushAlt,
							LabelPinnedBrush = PinnedBrush,
							Anchor = () => new RectangleF(Hud.Render.MinimapUiElement.Rectangle.X, Hud.Render.MinimapUiElement.Rectangle.Y + Hud.Render.MinimapUiElement.Rectangle.Height - MenuHeight, Hud.Render.MinimapUiElement.Rectangle.Width*0.85F, MenuHeight), 
							Alignment = HorizontalAlign.Left,
							Expand = MenuExpand.Up, 
						}
					},
					{ "TopCenter", new HorizontalMenuDock(Hud) {
							...
							LabelHoveredBrush = BgBrushAlt,
							LabelPinnedBrush = PinnedBrush,
							Anchor = () => new RectangleF(Hud.Window.Size.Width*0.5f - bottomCenterWidth*0.5f, 0, bottomCenterWidth, MenuHeight), //new RectangleF(0, 0, Hud.Window.Size.Width, MenuHeight), 
							Alignment = HorizontalAlign.Center, 
							Expand = MenuExpand.Down, 
						}
					},
					{ "TopRight", new VerticalMenuDock(Hud) { 
							...
							LabelHoveredBrush = BgBrushAlt,
							LabelPinnedBrush = PinnedBrush,
							Anchor = () => new RectangleF(Hud.Window.Size.Width - MenuHeight, 0, MenuHeight, Hud.Render.MinimapUiElement.Rectangle.Height), //MenuHeight*6 //new RectangleF(Hud.Window.Size.Width*0.5f, Hud.Window.Size.Height*0.5f, MenuHeight, MenuHeight*6), //
							Alignment = HorizontalAlign.Left,
					...
					{ "CenterRight", new VerticalMenuDock(Hud) { 
							...
							LabelHoveredBrush = BgBrushAlt,
							LabelPinnedBrush = PinnedBrush,
							Anchor = () => new RectangleF(Hud.Window.Size.Width - MenuHeight, Hud.Window.Size.Height*0.5f - Hud.Render.MinimapUiElement.Rectangle.Height*0.5f, MenuHeight, Hud.Render.MinimapUiElement.Rectangle.Height), 
							Alignment = HorizontalAlign.Center,
							Expand = MenuExpand.Left, 
						}
					},
					{ "CenterLeft", new VerticalMenuDock(Hud) { 
							...
							LabelHoveredBrush = BgBrushAlt,
							LabelPinnedBrush = PinnedBrush,
							Anchor = () => new RectangleF(0, Hud.Window.Size.Height*0.5f - Hud.Render.MinimapUiElement.Rectangle.Height*0.5f, MenuHeight, Hud.Render.MinimapUiElement.Rectangle.Height), 
							Alignment = HorizontalAlign.Center,
					...
					/*{ "Objectives", new HorizontalMenuDock(Hud) { 
							LabelBrush = BgBrush, 
							Anchor = () => new RectangleF(objectivesUI.Rectangle.X, objectivesUI.Rectangle.Y, objectivesUI.Rectangle.Width - MenuHeight, MenuHeight), 
							Alignment = HorizontalAlign.Right, 
							Expand = MenuExpand.Down,
						}
					},*/
				...
				//CalculateMenuHeight = () => (float)System.Math.Ceiling(Hud.Window.Size.Height - (potionUI.Rectangle.Bottom + 1)) + 1;
				//MenuHeight = CalculateMenuHeight();
				
				//float height = (float)System.Math.Ceiling(Hud.Window.Size.Height - y) + 1;
				//var uiRect = Hud.Render.GetPlayerSkillUiElement(ActionKey.Heal).Rectangle;
				...
				/*	
					...
					Docks = new Dictionary<string, IMenuDock>()
					{
						{ "BottomRight", new HorizontalMenuDock(Hud) { 
							LabelBrush = BgBrush, 
							Anchor = () => new RectangleF(potionUI.Rectangle.Right, potionUI.Rectangle.Bottom, Hud.Window.Size.Width - potionUI.Rectangle.Right, MenuHeight), 
							Alignment = HorizontalAlign.Right, 
							Expand = MenuExpand.Up, 
						...
						{ "BottomLeft", new HorizontalMenuDock(Hud) { 
							...
							Alignment = HorizontalAlign.Left, 
							Expand = MenuExpand.Up, 
							DebugBrush = DebugBrush } }, //bottomUI.Left + (bottomUI.Width * 0.09f) - lx
						{ "BottomCenter", new HorizontalMenuDock(Hud) { 
							BackgroundBrush = BgBrush, 
							Anchor = () => new RectangleF(healthBall.Rectangle.Right, bottomUI.Rectangle.Top + (bottomUI.Rectangle.Height * 0.335f), manaBall.Rectangle.Left - healthBall.Rectangle.Right, MenuHeight), 
							Alignment = HorizontalAlign.Center, 
							Expand = MenuExpand.Up, 
							DebugBrush = DebugBrush } },
						{ "TopCenter", new HorizontalMenuDock(Hud) { 
							LabelBrush = BgBrush, 
							Anchor = () => new RectangleF(0, 0, Hud.Window.Size.Width, MenuHeight), 
							Alignment = HorizontalAlign.Center, 
							Expand = MenuExpand.Down, 
							DebugBrush = DebugBrush } },
						{ "Objectives", new HorizontalMenuDock(Hud) { 
							...
							DebugBrush = DebugBrush } },
						{ "MinimapTop", new HorizontalMenuDock(Hud) { 
							Anchor = () => new RectangleF(minimapUI.Rectangle.X, minimapUI.Rectangle.Y, minimapUI.Rectangle.Width, MenuHeight), 
							Alignment = HorizontalAlign.Left,
							Expand = MenuExpand.Down, 
							DebugBrush = DebugBrush } },
						{ "MinimapBottom", new HorizontalMenuDock(Hud) { 
							Anchor = () => new RectangleF(minimapUI.Rectangle.X, minimapUI.Rectangle.Y + minimapUI.Rectangle.Height - MenuHeight, minimapUI.Rectangle.Width, MenuHeight), 
							Alignment = HorizontalAlign.Left,
							Expand = MenuExpand.Up, 
							DebugBrush = DebugBrush } }
						
						
						// { "BottomRight", new HorizontalMenuDock(Hud) { AnchorX = () => Hud.Window.Size.Width, AnchorY = () => y, Height = height, Alignment = HorizontalAlign.Right, ExpandUp = true, DebugBrush = DebugBrush, DropBox = new RectangleF(uiRect.Right, y, Hud.Window.Size.Width - uiRect.Right, height) } }, //Hud.Window.Size.Height - height //Width = Hud.Window.Size.Width - uiRect.Right
						// { "BottomCenter", new HorizontalMenuDock(Hud) { AnchorX = cx, AnchorY = cy, Height = height, Alignment = HorizontalAlign.Center, ExpandUp = true, DebugBrush = DebugBrush, DropBox = new RectangleF(healthBall.Right, cy, manaBall.Left - healthBall.Right, height) } },
						// { "BottomLeft", new HorizontalMenuDock(Hud) { AnchorX = lx, AnchorY = y, Height = height, Alignment = HorizontalAlign.Left, ExpandUp = true, DebugBrush = DebugBrush, DropBox = new RectangleF(lx, y, healthBall.Right - lx, height) } }, //bottomUI.Left + (bottomUI.Width * 0.09f) - lx
						// { "TopCenter", new HorizontalMenuDock(Hud) { AnchorX = Hud.Window.Size.Width*0.5f, AnchorY = 0, Height = height, Alignment = HorizontalAlign.Center, ExpandUp = false, DebugBrush = DebugBrush, DropBox = new RectangleF(0, 0, Hud.Window.Size.Width, height) } },
						// { "Inventory", new HorizontalMenuDock(Hud) { DrawState = (clip) => clip == ClipState.AfterClip && Hud.Inventory.InventoryMainUiElement.Visible, AnchorX = inventoryUI.X, AnchorY = feetUI.Bottom, Height = height, Alignment = HorizontalAlign.Left, ExpandUp = false, DebugBrush = DebugBrush, DropBox = new RectangleF(inventoryUI.X, feetUI.Bottom, inventoryUI.Width, height) } },
					};
				//}
				*/
				
				//Hud.Sound.Speak(inventoryUI.X.ToString());
				//Hud.Sound.Speak(inventoryUI.Y.ToString());
				
				foreach (string dockId in Docks.Keys)
					Docks[dockId].Id = dockId;
				
				//Hud.TextLog.Log("debug_IMenuAddon", "docks initialized", true, true);
				
				//foreach (IPlugin plugin in Hud.AllPlugins.Where(p => p is IMenuDock))
				//	Docks.Add(plugin.GetType().Name, plugin);
			...
			//var ui = Hud.Render.GetUiElement("Root.NormalLayer.rift_dialog_mainPage");
			//Hud.Sound.Speak(ui.Rectangle.Bottom.ToString());
		}
		...
		public void Log()
		{
			...
			System.IO.File.WriteAllText(string.Format("{0}\\{1}.cs", ConfigFilePath, ConfigFileName), TextBuilder.ToString());
		}
		...
		public void PinAddon(IMenuAddon addon)
		{
			...
			//{
				...
				if (PinnedAddons.ContainsKey(addon.Id)) //unpin
				{
					...
					//string id = addon.Id;
					//Mover.DeleteArea(PinnedAddons[addon.Id]);
