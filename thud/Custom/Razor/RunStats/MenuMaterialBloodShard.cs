namespace Turbo.Plugins.Razor.RunStats
{
	...
	public class MenuMaterialBloodShard : BasePlugin, IMenuAddon, IAfterCollectHandler//, ICustomizer /*, IInGameTopPainter, ILeftClickHandler, IRightClickHandler*/
	{
		...
		public void OnRegister(MenuPlugin plugin)
		{
			...
			MatFont = Hud.Render.CreateFont("tahoma", plugin.FontSize, 255, 255, 77, 0, false, false, 100, 0, 0, 0, true);
			...
			Label = new LabelRowDecorator(Hud,
				new LabelStringDecorator(Hud, () => ValueToString(Hud.Game.Me.Materials.BloodShard, (Hud.Game.Me.Materials.BloodShard < 10000 ? ValueFormat.NormalNumberNoDecimal : ValueFormat.ShortNumber))) {FuncCacheDuration = 1, Font = MatFont, Alignment = HorizontalAlign.Left}, //Timer.ElapsedMilliseconds > 0 ? ValueToString((long)((Gained + Spent) / ((decimal)Timer.ElapsedMilliseconds / 3600000)), ValueFormat.NormalNumberNoDecimal) + "/h" : "0/h") {FuncCacheDuration = 1, Font = MatFont, Alignment = HorizontalAlign.Left},
				new LabelTextureDecorator(Hud, Hud.Texture.GetItemTexture(Hud.Inventory.GetSnoItem(2603730171))) {TextureHeight = plugin.MenuHeight+6, ContentHeight = plugin.MenuHeight, ContentWidth = 20}
			) {/*Hint = plugin.CreateHint("Level")*/};
			LabelHint = plugin.CreateHint("Blood Shards");
			...
			Panel = new LabelColumnDecorator(Hud, 
				new LabelDelayedDecorator(Hud,
					new LabelAlignedDecorator(Hud, 
						new LabelStringDecorator(Hud, "BLOOD SHARD") {Font = plugin.TitleFont, SpacingLeft = 10, SpacingRight = 10},
				...
				new LabelAlignedDecorator(Hud,
					new LabelRowDecorator(Hud,
						new LabelTextureDecorator(Hud, Hud.Texture.GetItemTexture(Hud.Inventory.GetSnoItem(2603730171))) {TextureHeight = plugin.MenuHeight},
						new LabelStringDecorator(Hud, () => ValueToString(Hud.Game.Me.Materials.BloodShard, ValueFormat.NormalNumberNoDecimal)) {Font = MatFont}
		...
		public void AfterCollect()
		{
			...
			if (!init) // || IsSeasonal != Hud.Game.Me.Hero.Seasonal || IsHardcore != Hud.Game.Me.Hero.Hardcore)
			{
				...
				LastSeenCount = Hud.Game.Me.Materials.BloodShard;
			...
			if (Timer.IsRunning)
			{
				if (LastSeenCount != Hud.Game.Me.Materials.BloodShard)
				{
					if (GracePeriod)
					{
						Timer.Restart();
					...
					{
						if (LastSeenCount < Hud.Game.Me.Materials.BloodShard)
							Gained += (int)(Hud.Game.Me.Materials.BloodShard - LastSeenCount);
						else
							Spent += (int)(LastSeenCount - Hud.Game.Me.Materials.BloodShard);
