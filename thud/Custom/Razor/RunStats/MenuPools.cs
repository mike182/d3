namespace Turbo.Plugins.Razor.RunStats
{
	...
	public class MenuPools : BasePlugin, IMenuAddon, IAfterCollectHandler, INewAreaHandler, IInGameTopPainter /*, ICustomizer, ILeftClickHandler, IRightClickHandler*/
	{
		...
		public void AfterCollect()
		{
			...
			/*if (BonusPools == null)
				BonusPools = new Dictionary<uint, double>();
			if (Heroes == null)
				Heroes = new Dictionary<uint, HeroInfo>() { {Hud.Game.Me.HeroId, new HeroInfo(Hud.Game.Me)} };*/
			//if (HeroIndex == null)
			//	HeroIndex = Heroes.Values.OrderBy(h => h.Name).Select(h => h.Id).ToList();
			...
			foreach (IPlayer player in Hud.Game.Players)
			{
				...
				//bool added = !BonusPools.ContainsKey(player.HeroId);
				/*if (player.IsDead)
				{
					//if (BonusPools[player.HeroId] > 0)
					BonusPools[player.HeroId] = 0;
				}
				else if (player.IsMe || (player.HasValidActor && player.CoordinateKnown))
				{
					decimal xpToNext = player.CurrentLevelNormal < player.CurrentLevelNormalCap ? (decimal)Hud.Sno.GetExpToNextLevel(player) : (decimal)player.ParagonExpToNextLevel;
					double pools = xpToNext > 0 ? (double)((decimal)player.BonusPoolRemaining / xpToNext) * 10 : 0;
					BonusPools[player.HeroId] = pools;
					
					if (player.IsMe && !Heroes.ContainsKey(player.HeroId))
						Heroes.Add(player.HeroId, new HeroInfo(player));
				}*/
				
				/*if (!BonusPools.ContainsKey(player.HeroId) && player.IsMe)
				{
					//HeroInfo info = new HeroInfo(player);
					Heroes.Add(player.HeroId, new HeroInfo(player)); //info
					//InsertInOrder(player.HeroId); //HeroIndex.Add(HeroId, new HeroInfo(player));
				}*/
			}
		...
		public void PaintTopInGame(ClipState clipState)
		{
			...
			foreach (IPlayer player in Hud.Game.Players)
			{
				double pools = player.IsMe ? (Heroes.ContainsKey(player.HeroId) ? Heroes[player.HeroId].Pools : 0) : (BonusPools.ContainsKey(player.HeroId) ? BonusPools[player.HeroId] : 0);
				if (pools > 0) //BonusPools.ContainsKey(player.HeroId))
				{
					...
					PortraitUI.Paint(0, player.PortraitUiElement.Rectangle.Y + player.PortraitUiElement.Rectangle.Height*0.2f); //player.PortraitUiElement.Rectangle.Bottom - player.PortraitUiElement.Rectangle.Height*0.21f); //player.PortraitUiElement.Rectangle.X - 10 // - ((LabelTextureDecorator)PortraitUI.Labels[1]).Width
				}
			}
			
			//if (LocationPlugin.NearestWaypoint is object)
			//	LabelDecorator.DebugWrite(LocationPlugin.GetDisplayedName(Hud.Game.Me.SnoArea), Hud.Game.Me.ScreenCoordinate.X, Hud.Game.Me.ScreenCoordinate.Y);
			
			/*foreach (IActor actor in Hud.Game.Actors.Where(a => a.GizmoType == GizmoType.PoolOfReflection && a.Scene is object && a.Scene.SnoArea is object))
			{
				PoolInfo pool = Pools.Values.FirstOrDefault(p => p.Coordinate.Equals(actor.FloorCoordinate)); 
				//LabelDecorator.DebugWrite(LocationPlugin.GetDisplayedName(Hud.Game.Me.SnoArea), Hud.Game.Me.ScreenCoordinate.X, Hud.Game.Me.ScreenCoordinate.Y);
				LabelDecorator.DebugWrite(Hud.Game.Me.Scene.SnoScene.Code + " vs (pool)" + actor.Scene.SnoScene.Code, Hud.Game.Me.ScreenCoordinate.X, Hud.Game.Me.ScreenCoordinate.Y);
			}*/
		}
